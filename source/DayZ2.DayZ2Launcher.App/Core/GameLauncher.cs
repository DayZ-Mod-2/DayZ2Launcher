...
namespace DayZ2.DayZ2Launcher.App.Core
{
        ...
        IHandle<GameLauncher.LaunchStartGameEvent>
    {
        ...
        public MetaModDetails ModDetails => _modDetails;
        ...
        private void BeginLaunchProcess(Window parentWnd, Server server, string gameTypeIdent)
        {
            ...
            culledAddons.AddRange(
                ModDetails.AddOns.Where(
                    x =>
                    {
                        return gameType.AddOnNames.Count(y => x.Name.Equals(y, StringComparison.OrdinalIgnoreCase)) > 0;
                    })
                    .AsEnumerable());
            ...
            foreach (MetaPlugin plugin in enabledPlugins)
            {
                ...
                {
                    MessageBox.Show($"Could not find addon '{plugin.Addon}' referenced by plugin '{plugin.Ident}'",
                        "Internal error", MessageBoxButton.OK, MessageBoxImage.Error);
                    return;
                }
            ...
            _dlgWindow.Closed += (object sender, EventArgs e) =>
            {
                ...
                if (_dlgWindow.InstallSuccessful)
                {
                    new Thread(() =>
                    {
                        bool launchedGame = ActuallyLaunchGame(parentWnd, server, culledAddons, gameType);
                        if (launchedGame)
                        {
        ...
        private static bool ActuallyLaunchGame(Window parentWnd, Server server, IEnumerable<MetaAddon> addOns, MetaGameType gameType)
        {
            CloseGame();
