...
namespace DayZ2.DayZ2Launcher.App.Ui
{
	public class LaunchProgressViewModel : BindableBase
	{
		...
		public LaunchProgressViewModel(MetaGameType gameType)
		{
			...
			/*
			if (TorrentUpdater.Instance().CurrentState() != TorrentState.Seeding && TorrentUpdater.Instance().CurrentState() != TorrentState.Stopped)
			{
			...
			var thread = new Thread(() =>
			{
				...
				try
				{
					...
					foreach (MetaAddon addOn in addOns)
					{
						...
						string hash = Hash.HashFileSha256(filePath);
				...
				Closeable = true;
				Execute.OnUiThread(() => { OnRequestClose?.Invoke(this, null); }, Dispatcher);
			});
