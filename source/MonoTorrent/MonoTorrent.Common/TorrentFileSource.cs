...
namespace MonoTorrent.Common
{
	public class TorrentFileSource : ITorrentFileSource
	{
		...
		private void LoadFiles()
		{
			...
			{
				files.RemoveAll(
					delegate(string file) { return (new FileInfo(file).Attributes & FileAttributes.Hidden) == FileAttributes.Hidden; });
			}
			...
			Files = files.ConvertAll(delegate(string file) { return new FileMapping(file, file.Substring(fullPath.Length)); });
