	...
	internal static class SparseFile
	{
		...
		public static void CreateSparse(string filename, long length)
		{
			...
			try
			{
				...
				using (
					SafeFileHandle handle = CreateFileW(filename, access, sharing, IntPtr.Zero, creation, attributes, IntPtr.Zero))
				{
		...
		private static bool CanCreateSparse(string volume)
		{
			...
			bool result = GetVolumeInformationW(volume, volumeName, MAX_PATH, out serialNumber, out maxComponent, out fsFlags,
				systemName, MAX_PATH);
